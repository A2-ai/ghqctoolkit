# v0.8.0 - Unreleased
## Breaking Changes
* `--ipv4-only` is removed from `ghqc serve` and `ghqc ui`; there is no deprecated alias.
* `--bind <ADDR>` (env var `GHQC_BIND_ADDR`) replaces `--ipv4-only` on `ghqc serve` and `ghqc ui`, and accepts any IP address.
* The server now binds `127.0.0.1` by default instead of the dual-stack wildcard.
* POST /api/archive/generate: a file's QC metadata is now all-or-nothing. `milestone`, `approved`, `round`, and `subsequent_file_changes` must **all** be present or **all** absent; any partial combination is rejected with `400`. This widens the previous `milestone` + `approved` pairing, so a client posting those two alone now fails. Only the client knows which round it selected, and defaulting the missing fields would freeze a claim the archive cannot support.
* Removed from the GET /api/issues/status response: `commits`, `branch`, and `checklist_summary` at the top level, `issue.checklist_name`, and `qc_status.approved_commit` / `qc_status.initial_commit` / `qc_status.latest_commit`. All of them duplicated round-scoped values that could disagree with the round they described. Read `rounds[rounds.length - 1]` (and `rounds[0].start_commit` for the QC's original commit), `rounds[i].archive_commit`, and `drift` / `drift.newest_file_change` instead; `qc_status` is now a verdict (`status`, `status_detail`) and carries no commits.
* `rounds[i].archive_commit` is now nullable. It is `null` when the round's start commit could not be resolved on its branch, and when an approved round's approval is no longer among the commits it owns (force-push or rebase). A round in either state carries `placement: "unplaceable"` or an off-branch approval, and clients must render "fetch `<branch>`" rather than substituting a commit — the previous fallback silently archived a post-approval commit as approved content.
* GET /api/issues/status now returns a `branch_not_local` error entry for an issue whose **latest** round has no resolvable commit, instead of a status derived from a substituted commit. An earlier unplaceable round does not block status.
* POST /api/issues/:number/rounds: `notify` now defaults to `true` when omitted, matching `ghqc issue round create`. Clients relying on the previous `false` default must send `"notify": false` explicitly.
* `ghqc milestone archive` no longer aborts when one selected round's commit cannot be resolved: it skips that file, warns, and archives the rest. `ghqc_archive_metadata.json` gains a `skipped` array (`repository_file`, `round`, `branch`, `reason`) recording each omission, so a partial archive says so in its own manifest. The key is absent from a complete archive, so those manifests are unchanged and older archives still read. POST /api/archive/generate records the same omissions from a new optional `skipped` request array and echoes them in a new `skipped` response array — that path takes an explicit commit per file and no issue number, so only the client can decide what to skip. No path substitutes a commit: a file that cannot resolve is left out of `files` and declared in `skipped`, never archived at a stand-in commit.

## New Features
* POST /api/archive/generate accepts an optional `skipped` array (`repository_file`, `round`, `branch`, `reason`), written verbatim into the archive manifest's `skipped` record and reported back in the response, so the Web UI can archive the rest of a milestone when one file's selected round cannot be resolved — matching `ghqc milestone archive` — instead of refusing the whole archive. Omitting the field behaves exactly as before, and a complete archive's manifest is unchanged
* QC rounds: a QC issue can now go through multiple review cycles. `ghqc issue round create` starts a new round from the current checkout (branch and start commit are read-only, like `ghqc issue create`), posting a `# QC Round N` comment with a fresh checklist, optionally notifying the difference from the previous approval, and re-opening the issue; `ghqc issue status` prints a per-round table (branch, start, approval, checklist, preceding-gap size, divergent flag) before the latest round's detail; `ghqc milestone archive` prompts for the round to archive per QC-attached file, defaulting to the latest; POST /api/issues/:number/rounds backs the same flow for the Web UI
...
## Improvements
* An approved round whose approval commit was force-pushed or rebased off its branch now reports "Approval commit `<sha>` is no longer reachable on branch `<branch>`" instead of "Branch `<branch>` is not checked out locally" — the branch is present, so the old message sent users to fetch something they already had. The API still classifies it as `branch_not_local` with the same `branch` field, so clients need no change.
