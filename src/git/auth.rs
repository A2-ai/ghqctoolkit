...
pub enum AuthError {
    ...
    #[error("No authentication found. Try: GITHUB_TOKEN env var, 'gh auth login', or git credential manager")]
    NoAuth,
    #[error("IO error: {0}")]
...
fn create_authenticated_client_with_env(base_url: &str, env: &impl EnvProvider) -> Result<Octocrab, AuthError> {
...
fn get_git_credential_token(base_url: &str) -> Result<String, AuthError> {
    ...
    if let Some(stdin) = child.stdin.as_mut() {
        use std::io::Write;
        stdin.write_all(input.as_bytes()).map_err(|_| AuthError::NoAuth)?;
    }
    ...
    for line in output_str.lines() {
        if let Some(password) = line.strip_prefix("password=") {
            ...
            if password.starts_with("ghp_") || password.starts_with("github_pat_") || password.starts_with("gho_") || password.starts_with("ghu_") {
...
fn get_gh_config_dir_with_env(env: &impl EnvProvider) -> Result<PathBuf, AuthError> {
    ...
    if let Ok(home_dir) = env.var("HOME") {
        ...
        // Windows fallback
        Ok(PathBuf::from(user_profile).join("AppData").join("Roaming").join("GitHub CLI"))
    } else {
        Err(AuthError::NoAuth)
    }
}
...
mod tests {
    ...
    fn test_extract_host_from_url() {
        ...
        assert_eq!(extract_host_from_url("https://github.enterprise.com"), "github.enterprise.com");
        assert_eq!(extract_host_from_url("https://ghe.company.internal"), "ghe.company.internal");
    }
