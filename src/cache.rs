...
impl DiskCache {
    ...
    /// Generate a path for a specific cache file based on the operation/key
    ...
    pub fn write<T>(&self, key: &str, data: &T, use_ttl: bool) -> Result<(), Box<dyn std::error::Error>>
    where
        T: Serialize,
    {
        let path = self.path(key);
...
mod tests {
    ...
    fn test_path_generation() {
        ...
        assert_eq!(
            path,
            PathBuf::from("/tmp/cache/owner/repo/milestones.json")
        );
    }
    ...
    fn test_path_with_complex_names() {
        ...
        let path = cache.path("user_list");
        assert_eq!(
            path,
            PathBuf::from("/cache/my-org/my-repo_name/user_list.json")
        );
    }
