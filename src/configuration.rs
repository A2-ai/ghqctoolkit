...
impl Configuration {
    fn from_path(path: impl AsRef<Path>) -> Result<Self, ConfigurationError> {
        let path = path.as_ref();
        let options = ConfigurationOptions::from_path(path.join("options.yaml")).unwrap_or_default();
        Ok(Configuration {
...
fn parse_yaml_checklist(content: &str) -> Result<(String, String), ConfigurationError> {
    ...
    let mapping = yaml.as_mapping()
        .ok_or_else(|| ConfigurationError::InvalidYamlStructure("Root must be a mapping".to_string()))?;
    ...
    let title = title_key.as_str()
        .ok_or_else(|| ConfigurationError::InvalidYamlStructure("Checklist name must be a string".to_string()))?
        .to_string();
...
fn format_checklist_items_with_level(checklist: &Value, header_level: usize) -> Result<String, ConfigurationError> {
    match checklist {
        ...
        Value::Mapping(sections) => {
            ...
            for (section_key, section_value) in sections {
                let section_name = section_key.as_str()
                    .ok_or_else(|| ConfigurationError::InvalidYamlStructure("Section name must be a string".to_string()))?;
                
                match section_value {
                    // If the section contains a list, format it as items
                    Value::Sequence(items) => {
                        let formatted_section = format_section_list_with_level(section_name, items, header_level);
                    ...
                    Value::Mapping(_) => {
                        ...
                        let nested_content = format_checklist_items_with_level(section_value, header_level + 1)?;
...
fn format_section_list_with_level(section_name: &str, items: &[Value], header_level: usize) -> String {
...
mod tests {
    ...
    fn test_load_checklists_default() {
        ...
        insta::assert_snapshot!("simple_txt_checklist", &config.checklists["simple_checklist"]);
        insta::assert_snapshot!("backtick_txt_checklist", &config.checklists["Complex Checklist Name"]);
        insta::assert_snapshot!("simple_yaml_checklist", &config.checklists["Simple Tasks"]);
        insta::assert_snapshot!("hierarchical_yaml_checklist", &config.checklists["NCA Analysis"]);
        insta::assert_snapshot!("deeply_nested_yaml_checklist", &config.checklists["Complex Analysis"]);
    }
    ...
    fn test_configuration_options_with_custom_directory() {
        ...
        assert_eq!(config.options.prepended_checklist_notes, Some("Please review carefully".to_string()));
        assert_eq!(config.options.checklist_display_name, "Custom Quality Check");
        assert_eq!(config.options.logo_path, PathBuf::from("assets/custom_logo.svg"));
        assert_eq!(config.options.checklist_directory, PathBuf::from("my_custom_checklists"));
...
Second Checklist:
  - Item 2"#;
        ...
        assert!(matches!(result.unwrap_err(), ConfigurationError::InvalidYamlStructure(_)));
